...
- (void)initAssistantMenuItems {
    ...
    //        设置消息获取url
    NSMenuItem *messageSyncUrl = [NSMenuItem menuItemWithTitle:@"消息同步"
                                                       action:@selector(onSetSyncUrl:)
                                                       target:self
                                                keyEquivalent:@""
                                                        state:[[TKWeChatPluginConfig sharedConfig] syncUrlEnable]];
...
- (void)onSetSyncUrl:(NSMenuItem *)item {
    
    NSTextField *input = [[NSTextField alloc] initWithFrame:NSMakeRect(0, 0, 200, 24)];
    NSString *syncUrl = [[TKWeChatPluginConfig sharedConfig] syncUrl];
    if(syncUrl){
        input.stringValue = syncUrl;
    }
    NSAlert *alert = [[NSAlert alloc] init];
    [alert setAccessoryView:input];
    [alert setMessageText:@"设置同步Url"];
    [alert setInformativeText:@"请输入消息同步url."];
    ...
    if (button == NSAlertSecondButtonReturn) {
       // [input validateEditing];
        ...
        if([text length] != 0){
            NSLog(@"%@", text);
            [[TKWeChatPluginConfig sharedConfig] setSyncUrl:text];
            [[TKWeChatPluginConfig sharedConfig] setSyncUrlEnable:true];
            item.state = 1;
        }else{
            [[TKWeChatPluginConfig sharedConfig] setSyncUrl:@""];
            [[TKWeChatPluginConfig sharedConfig] setSyncUrlEnable:false];
            item.state = 0;
        }
    }else{
