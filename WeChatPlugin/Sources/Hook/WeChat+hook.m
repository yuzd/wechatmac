...
+ (void)setUpSyncMemberThread {
    //每5分钟同步一次通讯录列表
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        
        //获取通讯录所有的列表 然后放入到一个txt文档
        ...
        [[[NSOperationQueue alloc] init] addOperationWithBlock:^{
            ...
            @try {
                ...
                [allList writeToFile:userSaveaPath atomically:YES];
...
- (void)autoReplyWithMsg:(AddMsg *)addMsg {
    ...
    [autoReplyModels enumerateObjectsUsingBlock:^(TKAutoReplyModel *model, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([model.replyContent hasPrefix:@"http"] && [msgContact isGroupChat]) {
            //是群聊 并且是开启了回复到指定的url
            //查看是否是@我自己了
            @try {
                NSString *pushContent = [addMsg valueForKey: @"pushContent"];
                if ([pushContent containsString:@"@了你"])
                {
                    [self replyWithMsgPost:addMsg model:model];
                }
            }
            @catch (NSException *exception) {
...
- (void)replyWithMsgPost:(AddMsg *)addMsg model:(TKAutoReplyModel *)model {
    ...
    [request setURL:[NSURL URLWithString:randomReplyContent]];
    [request setValue:@"application/json" forHTTPHeaderField:@"Accept"];
    [request setValue:@"application/json" forHTTPHeaderField:@"Content-type"];
    
    // Forming string with credentials 'myusername:mypassword'
    NSString *authStr = [NSString stringWithFormat:@"%@:%@", @"cloudbag", @"bagcloud@2016"];
    ...
    //拿到消息内容 发送的人 发送人id 群名称 群id 然后post到指定的url
    ...
    NSData *data2 = [NSJSONSerialization dataWithJSONObject:contentDictionary options:NSJSONWritingPrettyPrinted error:nil];
    NSString* requestBodyString = [[NSString alloc] initWithData:data2
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSString *msg =jsonDict[@"Data"];
         [[TKMessageManager shareManager] sendTextMessage:msg toUsrName:addMsg.fromUserName.string delay:delayTime];
    }] resume];
}


- (void)GetMessageQueue{
    NSString *url = [[TKWeChatPluginConfig sharedConfig] syncUrl];
    if(!url || [url length] == 0 ){
        return;
    }
    NSString *currentUserName = [objc_getClass("CUtility") GetCurrentUserName];
    ...
    [[session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        ...
        NSData * responseData = [requestReply dataUsingEncoding:NSUTF8StringEncoding];
        NSDictionary *jsonDict = [NSJSONSerialization JSONObjectWithData:responseData options:kNilOptions error:&error];
        NSLog(@"GetMessageQueue: %@", jsonDict);
        
        NSString *msg =jsonDict[@"Data"];
        NSString *user =jsonDict[@"Info"];
        
        if(msg!=nil && user!=nil && [msg length] != 0 && [user length] != 0){
            [[TKMessageManager shareManager] sendTextMessage:msg toUsrName:user delay:0];
        }
